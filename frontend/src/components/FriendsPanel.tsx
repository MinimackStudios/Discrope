...
const FriendsPanel = ({
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4" onClick={onClose}>
      <section className="w-full max-w-2xl rounded-lg bg-[#2b2d31] p-4" onClick={(e) => e.stopPropagation()}>
        <div className="mb-3 flex items-center justify-between">
          <h2 className="text-lg font-semibold">Friends</h2>
          <button className="text-sm text-discord-muted hover:text-white" onClick={onClose}>Close</button>
        </div>
        ...
        <form onSubmit={submit} className="mb-4 flex gap-2">
          ...
          />
          <button className="rounded bg-discord-blurple px-3 py-2 text-sm font-semibold text-white">Send</button>
        </form>
            ...
            <div className="space-y-1">
              {pending.map((request) => (
                <div key={request.id} className="flex items-center gap-2 rounded bg-[#1e1f22] px-2 py-2 text-sm">
                  ...
                  <button className="text-xs text-[#23a55a]" onClick={() => void onAccept(request.id)}>Accept</button>
                  <button className="text-xs text-[#ed4245]" onClick={() => void onReject(request.id)}>Decline</button>
                </div>
              ))}
            ...
            <div className="space-y-1">
              {friends.map((friend) => (
                <div key={friend.id} className="flex items-center gap-2 rounded bg-[#1e1f22] px-2 py-2 text-sm">
                  ...
                  <button className="text-xs text-[#00a8fc]" onClick={() => void onStartDM(friend.id)}>Message</button>
                  <button className="text-xs text-[#ed4245]" onClick={() => void onRemoveFriend(friend.id)}>Remove</button>
                </div>
              ))}
